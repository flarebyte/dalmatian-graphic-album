...
import { Assertion, Metadata } from './schema/dalmatian_generated';
...
const discardNullAssertions = (assertions: ReadonlyArray<Assertion | null>):  ReadonlyArray<Assertion> => 
  assertions.filter(notNullAssertion).map(a => a===null ? voidAssertion : a)

const findAssertions = (iri: ResourceIdentifier) => (metadata: Metadata | null): ReadonlyArray<Assertion> => {
